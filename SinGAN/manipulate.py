...
def SinGAN_generate_signal(Gs, Zs, reals, NoiseAmp, opt, in_s=None, scale_v=1, scale_h=1, n=0, start_scale=0,
                           num_samples=50, alpha=0.1, beta=0.9):
        ...
        for G, Z_opt, noise_amp in zip(Gs, Zs, NoiseAmp):
            ...
            for i in range(0, num_samples, 1):
                ...
                if n == len(reals) - 1:
                    if opt.mode == 'train':
                        dir2save = '%s/RandomSamples/%s/gen_start_scale=%d' % (
                        opt.out, opt.input_name[:-4], start_scale)
                    else:
                        dir2save = functions.generate_dir2save(opt)
                    try:
                        os.makedirs(dir2save)
