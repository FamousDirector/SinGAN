...
def SinGAN_generate_signal(Gs,Zs,reals,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,start_scale=0,num_samples=50,alpha=0.1,beta=0.9):
    if opt.mode == 'animation':
            ...
            for i in range(0, num_samples, 1):
                ...
                if (count == len(Gs) - 1):
                    ...
                    try:
                        ...
                        pass
                    np.savez('%s/%d.npz' % (dir2save, i), I_curr)
        ...
        for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
            ...
            for i in range(0,num_samples,1):
                ...
                if n == len(reals)-1:
                    if opt.mode == 'train':
                        dir2save = '%s/RandomSamples/%s/gen_start_scale=%d' % (opt.out, opt.input_name[:-4], start_scale)
                    else:
                        dir2save = functions.generate_dir2save(opt)
                    try:
                        os.makedirs(dir2save)
                        ...
                        pass
                    if (opt.mode != "harmonization") & (opt.mode != "editing") & (opt.mode != "SR") & (opt.mode != "paint2image"):
                        np.savez('%s/%d.npz' % (dir2save, i), functions.convert_image_np(I_curr.detach()))
                images_cur.append(I_curr)
            n += 1
