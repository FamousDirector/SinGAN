...
def generate_dir2save(opt):
    dir2save = None
    if (opt.mode == 'train'):
        dir2save = 'TrainedModels/%s_row%d/t=%d/%s/scale_factor=%f,alpha=%d' % (opt.input_name[:-4], opt.row,
                                                                                opt.sample_offset,
                                                                                opt.spectral_type,
                                                                                opt.scale_factor_init, opt.alpha)
    elif opt.mode == 'random_samples':
        dir2save = '%s/RandomSamples/%s_row%d/t=%d/%s/gen_start_scale=%d' % (opt.out, opt.input_name[:-4], opt.row,
                                                                             opt.sample_offset,
                                                                             opt.spectral_type, opt.gen_start_scale)
    elif (opt.mode == 'animation_train'):
        dir2save = 'TrainedModels/%s_row%d/t=%d/%s/scale_factor=%f_noise_padding' % (opt.input_name[:-4], opt.row,
                                                                                     opt.sample_offset,
                                                                                     opt.spectral_type,
                                                                                     opt.scale_factor_init)
    elif opt.mode == 'animation':
        dir2save = '%s/Animation/%s_row=%d/t=%d/%s/start_scale=%d/alpha=%f_beta=%f' % (
        opt.out, opt.input_name[:-4], opt.row,
        opt.sample_offset,
        opt.spectral_type,
        opt.gen_start_scale,
        opt.animation_alpha,
